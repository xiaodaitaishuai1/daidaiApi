...
function ratioToBasePrice(ratio: unknown): string {
  ...
  if (num === null) return ''
  return formatPriceForDisplay(num * 2)
}
...
function deriveLanePrice(
  ...
  return formatPriceForDisplay(ratioNumber * denominatorNumber)
}
...
export function ModelPricingEditorPanel({
  ...
  const deriveLaneRatio = (
    ...
    if (lane === 'audioOutput') {
      ...
      return formatRatioForStorage(priceNumber / audioInputPrice)
    }

    const inputPrice = toNumberOrNull(nextPromptPrice)
    if (inputPrice === null || inputPrice === 0) return ''
    return formatRatioForStorage(priceNumber / inputPrice)
  }
  ...
  const syncLaneRatios = (
    ...
    setFormValue(
      'ratio',
      inputPrice !== null ? formatRatioForStorage(inputPrice / 2) : ''
    )
