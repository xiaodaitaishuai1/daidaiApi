...
export function AccessTokenDialog({
  ...
  const handleGenerate = async () => {
    if (await generate()) setConfirmOpen(false)
  }
          ...
          {token ? (
            ...
            <Empty className='border py-8'>
              <EmptyHeader>
                ...
                <EmptyTitle>{t('Access tokens are shown only once')}</EmptyTitle>
                <EmptyDescription>
                  {t(
                ...
                <EmptyDescription>
                  {t('Regenerating immediately invalidates any existing token.')}
                </EmptyDescription>
              </EmptyHeader>
            </Empty>
          )}
